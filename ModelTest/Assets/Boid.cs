...
public class Boid : MonoBehaviour {
    ...
    [Header("ChannelFinding")]
    public bool channelFindingEnabled = false;
    public float channelWidth = 10.0f;
    public float channelDistance = 20.0f;
    public float channelTurnStrength = 2.0f;
    private Vector3 targetChannel = Vector3.zero;
    ...
    public void MakeFeelers()
    {
        ...
        MakeFeeler(0, Vector3.forward);
        MakeFeeler(45, Vector3.up);
        MakeFeeler(-45, Vector3.up);
        MakeFeeler(45, Vector3.right);
        MakeFeeler(-45, Vector3.right);

    }

    private void MakeFeeler(int angle, Vector3 dir)
    {
        Vector3 newFeeler = Vector3.forward * feelerDistance;
        newFeeler = Quaternion.AngleAxis(angle, dir) * newFeeler;
        newFeeler = transform.TransformPoint(newFeeler);
        planeAvoidanceFeelers.Add(newFeeler);
    }
    ...
    public Vector3 PlaneAvoidance()
    {
        ...
        //can't pass all the planes in the world in beforehand, so raycasting is required
        foreach (Vector3 feeler in planeAvoidanceFeelers)
        {
            Ray ray = new Ray(transform.position, feeler);
            RaycastHit info;

            if (Physics.Raycast(ray, out info, feelerDistance))
            {
                planeAvoidanceActive = true;
                force += info.normal * info.distance;
            }
        }
    ...
    public Vector3 ChannelFinding()
    {
        Vector3 force = Vector3.zero;

        if (currentMagnitude < 0.1f)//if it's not switching channel
        {
            //first, see if the way ahead is blocked
            Ray checkAhead = new Ray(transform.position, transform.forward);
            RaycastHit info;

            if (Physics.Raycast(checkAhead, out info, channelDistance))
            {
                //if it is, chack the surrounding channels
                Vector3 above = transform.up * channelWidth;
                Vector3 below = -transform.up * channelWidth;
                Vector3 left = -transform.right * channelWidth;
                Vector3 right = transform.right * channelWidth;

                Ray checkAbove = new Ray(transform.position + above, transform.forward);
                Ray checkBelow = new Ray(transform.position + below, transform.forward);
                Ray checkLeft = new Ray(transform.position + left, transform.forward);
                Ray checkRight = new Ray(transform.position + right, transform.forward);

                RaycastHit channelInfo;

                if (!Physics.Raycast(checkLeft, out channelInfo, channelDistance))
                {
                    //return left * info.distance * channelTurnStrength;
                    //targetChannel = left;
                    targetChannel = left * info.distance;
                    currentMagnitude = 2.0f;
                }

                if (!Physics.Raycast(checkRight, out channelInfo, channelDistance))
                {
                    //return right * info.distance * channelTurnStrength;
                    //targetChannel = right;
                    targetChannel = right * info.distance;
                    currentMagnitude = 2.0f;
                }

                if (!Physics.Raycast(checkAbove, out channelInfo, channelDistance))
                {
                    //return above * info.distance * channelTurnStrength;
                    //targetChannel = above;
                    targetChannel = above * info.distance;
                    currentMagnitude = 2.0f;
                }

                /*if (!Physics.Raycast(checkBelow, out channelInfo, channelDistance))
                {
                    //return below * info.distance * channelTurnStrength;
                    //targetChannel = below;
                    targetChannel = below * info.distance;
                    currentMagnitude = 2.0f;
                }*/
                //had a tendency to nosedive when things got crowded
            }
        }
        else
        {
            force = targetChannel;
            Vector3.ClampMagnitude(targetChannel, currentMagnitude);
            currentMagnitude -= (channelTurnStrength * Time.deltaTime);
        }
    ...
    {
        seekEnabled = arriveEnabled = fleeEnabled = pursueEnabled = offsetPursueEnabled = pathFollowEnabled = wanderEnabled = planeAvoidanceEnabled = channelFindingEnabled = false;
    }
    ...
    void OnDrawGizmos()
    {
        ...
        if (planeAvoidanceEnabled && planeAvoidanceActive)
        {
            foreach (Vector3 feeler in planeAvoidanceFeelers)
            {
                ...
                Gizmos.DrawLine(transform.position, feeler * feelerDistance);
            }
        }
    ...
    void Update()
    {
        ...
        if (channelFindingEnabled)
        {
            force += ChannelFinding();
        }
